...
export default function BannerLGPD() {
	...
	return (
		<div
			className={`fixed bottom-0 center ${
				!cookieConsent ? 'flex' : 'hidden'
			}  items-center w-full mx-auto h-fit text-brand-neutral-100 text-xs lg:text-sm p-3  z-[999999999999999]`}>
			<div className=' max-w-[1024px]  rounded-lg gap-1 lg:gap-5 shadow-lg flex flex-col lg:flex-row justify-center items-center mx-auto p-5 bg-brand-neutral-950 lg:py-8 lg:px-8'>
				...
				<div className='gap-3 lg:gap-5 flex justify-center items-center mx-auto'>
					<button
						...
						type='button'
						className={`w-fit px-4 py-2 lg:w-28 lg:h-12 bg-brand-red-500 rounded-full z-50 disabled:grayscale disabled:hover:bg-brand-red-300 flex gap-2 items-center justify-center text-center hover:bg-brand-red-300  transition duration-300 mt-3 lg:text-base text-xs font-bold tracking-wide`}>
