	...
	if (role === 'CTA') {
		return (
			<motion.a
				...
				className={`p-4 bg-bgButton bg-btnSize bg-left hover:bg-right cursor-pointer rounded flex gap-2 items-center text-lg transition-all ease-linear duration-500 w-fit text-center text-md font-bold tracking-wide font-montserratTitle`}
				href={link}
				target='_blank'
				referrerPolicy='no-referrer'>
				{text}
	...
	if (role === 'form') {
		return (
			<button
				disabled={disabled}
				type='submit'
				className={`p-3 bg-bgButtonHero bg-btnSize bg-left hover:bg-right rounded z-50 disabled:grayscale disabled:hover:bg-left flex gap-2 items-center justify-center w-full text-centertext-md transition-all duration-500 mt-3 text-lg font-bold tracking-wide font-montserratTitle cursor-pointer`}>
				{loading ? (
					<LoadingBtn />
				) : (
